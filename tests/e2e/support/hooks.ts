...
BeforeAll({ timeout: 60000 }, async function () {
  ...
  if (isProduction) {
    console.log("Starting preview server (build should already be complete)...");
    ...
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        if (!resolved) {
          console.error("Preview server timeout - promise never resolved!");
      ...
      devServer!.on("error", (error) => {
        console.error("Preview server spawn error:", error);
        if (!resolved) {
          resolved = true;
          clearTimeout(timeout);
          reject(error);
